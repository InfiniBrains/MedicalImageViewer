            ...
            DicomJpegParams parameters)
        {
            ...
            for (var frame = 0; frame < oldPixelData.NumberOfFrames; frame++)
            {
                ...
                for (var c = 0; c < nc; c++)
                {
                    ...
                    {
                        throw new InvalidOperationException( "JPEG codec does not support Bits Allocated == "+oldPixelData.BitsAllocated);
                    }
                }
            ...
            for (var frame = 0; frame < oldPixelData.NumberOfFrames; frame++)
            {
                ...
                using (var stream = new MemoryStream(jpegData.Data))
                {
                    ...
                    for (var c = 0; c < decoder.ComponentsPerSample; c++)
                    {
                        ...
                        {
                            throw new InvalidOperationException("JPEG module does not support Bits Allocated == "+newPixelData.BitsAllocated+"!");
                        }
                    }
