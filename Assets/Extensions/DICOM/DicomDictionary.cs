...
namespace Dicom
{
    ...
    {
		public static byte[] xmlDicomDictionary, xmlPrivateDictionary;

		public static void LoadDictionaries()
		{
			xmlDicomDictionary =  ((UnityEngine.TextAsset)UnityEngine.Resources.Load("DICOM Dictionary")).bytes;
			xmlPrivateDictionary = ((UnityEngine.TextAsset)UnityEngine.Resources.Load("Private Dictionary")).bytes;
		}
        ...
        public static DicomDictionary EnsureDefaultDictionariesLoaded(bool? loadPrivateDictionary = null)
        {
            ...
            lock (_lock)
            {
                if (_default == null)
                {
                    ...
                    {
#if NET35
                        if (xmlDicomDictionary == null)
                            LoadDictionaries();
						using (var stream = new MemoryStream(xmlDicomDictionary))
						{
							var reader = new DicomDictionaryReader(dict, DicomDictionaryFormat.XML, stream);
							reader.Process();
						}
#else
                        var assembly = typeof(DicomDictionary).Assembly;
                        using (
                    ...
                    if (loadPrivateDictionary.GetValueOrDefault(true))
                    {
                        ...
                        {
#if NET35
                            if (xmlPrivateDictionary == null)
                                LoadDictionaries();
							using (
								var stream = new MemoryStream(xmlPrivateDictionary))
							{
								var reader = new DicomDictionaryReader(dict, DicomDictionaryFormat.XML, stream);
								reader.Process();
							}
