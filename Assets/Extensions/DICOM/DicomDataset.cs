    ...
    public class DicomDataset : IEnumerable<DicomItem>
    {
        ...
        private T Get<T>(DicomTag tag, int n, bool useDefault, T defaultValue)
        {
            ...
            if (item.GetType().IsSubclassOf(typeof(DicomElement)))
            {
                ...
                if (!typeof(T).IsArray && (n >= element.Count || element.Count == 0))
                {
                    if (useDefault) return defaultValue;
        ...
        private DicomDataset DoAdd<T>(DicomVR vr, DicomTag tag, IList<T> values, bool allowUpdate)
        {
            ...
            if (vr == DicomVR.CS)
            {
                ...
                if (typeof(T).IsEnum) return DoAdd(new DicomCodeString(tag, values.Select(x => x.ToString()).ToArray()), allowUpdate);
            }
            ...
            throw new InvalidOperationException("Unable to create DICOM element of type "+vr.Code+" with values of type "+typeof(T));
        }
            ...
            out IEnumerable<TOut> parsedValues)
        {
            ...
            if (typeof(T) == typeof(string))
            {
                ...
                parsedValues = stringValues.Where(n => !string.IsNullOrEmpty(n!=null?n.Trim():null)).Select(parser);
