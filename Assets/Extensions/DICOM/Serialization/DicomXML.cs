...
namespace Dicom.Serialization
{
    ...
    public static class DicomXML
    {
        ...
        private static void WriteDicomAttribute(StringBuilder xmlOutput, DicomItem item)
        {
            ...
            {
                xmlOutput.AppendLine(@"<DicomAttribute tag="+item.Tag.Group+""+item.Tag.Element+" vr="+item.ValueRepresentation.Code+" keyword="+item.Tag.DictionaryEntry.Keyword+" privateCreator=" +item.Tag.PrivateCreator.Creator+">");
            }
            else
            {
                xmlOutput.AppendLine(@"<DicomAttribute tag="+item.Tag.Group+""+item.Tag.Element+" vr="+item.ValueRepresentation.Code+" keyword="+item.Tag.DictionaryEntry.Keyword+">");
            }
        }
